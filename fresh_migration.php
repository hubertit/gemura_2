...
// Fresh migration script to register ALL users from old system to new v2 API
...
echo "Starting fresh migration to v2 API...\n\n";
...
function registerUser($userData) {
    ...
    $postData = [
        "name" => $userData["name"],
        "phone" => $userData["phone"], // Use phone as-is, no +250 prefix
        "password" => "Pass123", // Default password for all users
        "role" => $userData["role"],
...
foreach ($users as $index => $user) {
    ...
    $userData = [
        ...
        "phone" => $user['user_phone'], // Use phone as-is
            ...
            if ($token && !empty($user['suppliers'])) {
                ...
                foreach ($user['suppliers'] as $supplierIndex => $supplier) {
                    $supplierData = [
                        ...
                        "phone" => $supplier['user_phone'], // Use phone as-is
...
file_put_contents('fresh_migration_results.json', json_encode($results, JSON_PRETTY_PRINT));

echo "\nMigration results saved to: fresh_migration_results.json\n";
