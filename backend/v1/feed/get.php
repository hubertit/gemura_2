...
if ($post_id) {
    ...
    $query = mysqli_query($connection, "
        SELECT p.*, 
               ...
               CASE WHEN bi.id IS NOT NULL THEN 1 ELSE 0 END as is_bookmarked
        FROM feed_posts p
        JOIN users u ON p.user_id = u.id
        LEFT JOIN feed_interactions li ON p.id = li.post_id 
            AND li.user_id = $current_user_id AND li.interaction_type = 'like'
        LEFT JOIN user_bookmarks bi ON p.id = bi.post_id 
            AND bi.user_id = $current_user_id
    ...
    // Get all feed posts (no following filter)
    $query = mysqli_query($connection, "
        SELECT p.*, 
               u.name as user_name, u.account_type, u.kyc_status,
               CASE WHEN li.id IS NOT NULL THEN 1 ELSE 0 END as is_liked,
               CASE WHEN bi.id IS NOT NULL THEN 1 ELSE 0 END as is_bookmarked
        FROM feed_posts p
        JOIN users u ON p.user_id = u.id
