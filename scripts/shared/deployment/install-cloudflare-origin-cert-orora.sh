#!/usr/bin/env bash
# Install Cloudflare Origin certificate for app.orora.rw on the Kwezi server.
# Requires: cert and key in scripts/shared/deployment/ssl/orora-origin.crt and .key
#           server-credentials.sh (SERVER_PASS, SERVER_IP, SERVER_USER)
#
# Usage: ./scripts/shared/deployment/install-cloudflare-origin-cert-orora.sh
...
CERT_SRC="$SSL_DIR/orora-origin.crt"
KEY_SRC="$SSL_DIR/orora-origin.key"
...
echo "═══════════════════════════════════════════════════════════════"
echo "  Install Cloudflare Origin cert for app.orora.rw"
...
sshpass -p "$SERVER_PASS" ssh -o StrictHostKeyChecking=no "$SERVER_USER@$SERVER_IP" "chmod 644 $REMOTE_SSL_DIR/orora-origin.crt && chmod 600 $REMOTE_SSL_DIR/orora-origin.key"

# Upload updated nginx config (includes both Gemura and Orora 443 blocks)
...
echo ""
echo "✅ Cloudflare Origin certificate installed for app.orora.rw"
echo "   HTTPS: https://app.orora.rw (port 443)"
echo "   Set Cloudflare SSL/TLS to Full or Full (strict) for orora.rw."
echo "═══════════════════════════════════════════════════════════════"
