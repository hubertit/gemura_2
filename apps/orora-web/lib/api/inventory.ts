...
/** Movement with product_name (from GET /inventory/movements) */
...
export const inventoryApi = {
  ...
  getProductMovements: async (
    productId: string,
    params?: { page?: number; limit?: number; account_id?: string; movement_type?: string; date_from?: string; date_to?: string }
  ...
  getAllMovements: async (
    params?: { page?: number; limit?: number; account_id?: string; product_id?: string; movement_type?: string; date_from?: string; date_to?: string }
  ): Promise<{ code: number; status: string; message: string; data: { items: InventoryMovementWithProduct[]; pagination: { page: number; limit: number; total: number; total_pages: number } } }> => {
    const search = new URLSearchParams();
    if (params?.page != null) search.append('page', String(params.page));
    if (params?.limit != null) search.append('limit', String(params.limit));
    if (params?.account_id) search.append('account_id', params.account_id);
    ...
    if (params?.date_to) search.append('date_to', params.date_to);
    const q = search.toString();
    return apiClient.get(`/inventory/movements${q ? `?${q}` : ''}`);
  },
  ...
  downloadTemplate: async (): Promise<void> => {
    const token = typeof window !== 'undefined' ? localStorage.getItem('orora-auth-token') : null;
