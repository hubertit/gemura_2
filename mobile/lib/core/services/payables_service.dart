...
class PayablesService {
  ...
  Future<PayablesSummary> getPayables({
    ...
    try {
      ...
      if (supplierAccountId != null) queryParams['supplier_account_id'] = supplierAccountId;
      if (dateFrom != null) queryParams['date_from'] = dateFrom;
      if (dateTo != null) queryParams['date_to'] = dateTo;
      if (paymentStatus != null) queryParams['payment_status'] = paymentStatus;

      print('📊 Fetching payables');
      print('🌐 API URL: ${AppConfig.apiBaseUrl}/accounting/payables');
      ...
      print('✅ Payables response: ${response.data}');

      if (response.data['code'] == 200 && response.data['data'] != null) {
        return PayablesSummary.fromJson(response.data['data']);
      } else {
        throw Exception('Invalid response format');
      }
    } on DioException catch (e) {
      print('❌ Payables DioException: ${e.message}');
      print('❌ Response: ${e.response?.data}');
      throw _handleDioError(e);
    } catch (e) {
      print('❌ Payables Exception: $e');
      throw Exception('Failed to fetch payables: $e');
    }
  }

  /// Record payment for a collection
  Future<Map<String, dynamic>> recordPayment({
    ...
    try {
      ...
      print('💰 Recording payment for collection: $collectionId');
      print('🌐 API URL: ${AppConfig.apiBaseUrl}/collections/$collectionId/payment');

      final response = await AuthenticatedDioService.instance.post(
        '/collections/$collectionId/payment',
        data: {
          'amount': amount,
